...
open class WebStarterLibrariesStep(contextProvider: WebStarterContextProvider) : ModuleWizardStep() {
  ...
  private val wizardContext: WizardContext = contextProvider.wizardContext
  ...
  private val frameworkVersionProperty: GraphProperty<WebStarterFrameworkVersion?> = propertyGraph.property(null)
    ...
    return panel {
      ...
      row {
        label(messages?.dependenciesLabel ?: JavaStartersBundle.message("title.project.dependencies.label"))
          .applyToComponent { labelFor = librariesList }
      }
  ...
  protected open fun createSelectedLibrariesPanel(): SelectedLibrariesPanel {
    ...
    for (category in dependencyCategories) {
      ...
      for (dependency in category.dependencies) {
        ...
        if (search.isBlank() || isDependencyMatched(dependency, search)) {
          ...
          libraryNode.isEnabled = !dependency.isDefault && state is DependencyAvailable
  ...
  private fun updateAvailableDependencies() {
    ...
    walkCheckedTree(root) {
      ...
      if (dependency != null) {
        val state = getDependencyState(dependency)
        it.isEnabled = !dependency.isDefault && state is DependencyAvailable
      }
    }
    librariesList.repaint()
